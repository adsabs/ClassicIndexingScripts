...
# ===================================
#
# MANUAL OPERATION: 2024 May09 [MT]
# Use this file to generate input files for docmatching without sending the
# resulting data to docmatch.  This will be used to manually prepare input
# files for multiple independent instances of docmatching for extremely large
# ingests.
...
curind="$ADS_ABSTRACTS/$db/index/done.2024-12-29"
...
recordcount="$((`wc -l < $ilist`))"
nmax=60
splitsize=$(($recordcount/$nmax))
remainder=$(($recordcount % $nmax))
echo "The file with $recordcount lines will be split into $nmax files of size $splitsize with remainder $remainder"


for i in $(seq 1 1 $(($nmax-1))); do
  headread=$(($i*$splitsize))
  splitfile="$ilist.$i"
  echo $headread
  echo $splitfile
  head -$headread $ilist | tail -$splitsize > $splitfile
done

splitfile="$ilist.$nmax"
tailread=$(($splitsize+$remainder))
tail -$tailread $ilist > $splitfile
